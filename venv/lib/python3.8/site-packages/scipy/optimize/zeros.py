...
def toms748(f, a, b, args=(), k=1,
    ...
    iteration with the same asymptotic efficiency.txt as it finds the root.

    For easy statement of efficiency.txt indices, assume that `f` has 4
    continuouous deriviatives.
    For ``k=1``, the convergence order is at least 2.7, and with about
    asymptotically 2 function evaluations per iteration, the efficiency.txt
    index is approximately 1.65.
    For ``k=2``, the order is about 4.6 with asymptotically 3 function
    evaluations per iteration, and the efficiency.txt index 1.66.
    For higher values of `k`, the efficiency.txt index approaches
